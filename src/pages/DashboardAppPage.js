...
// sections
import AppCurrentDataBase from '../sections/@dashboard/app/AppCurrentDataBase';
import {
  AppTasks,
  AppNewsUpdate,
  AppOrderTimeline,
...
export default function DashboardAppPage() {
  ...
  const dataBasePie = [...dataBase].map((data) => {
    return {
      label: `${data.BD} - ( MySql ) `,
      value: Number(parseFloat(data.MB).toFixed(1)),
    };
  });
      ...
      <Container maxWidth="xl">
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6} lg={4}>
            <AppCurrentDataBase
              title="Espacio de la base de datos"
              chartData={dataBasePie}
              chartColors={[
